...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/12 14:23:46 by sbudilko          #+#    #+#             */
/*   Updated: 2018/06/12 14:23:48 by sbudilko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_xx_conv_4(int fd, t_syntax *syn, char *str, int len_pre)
{
	char	str2[len_pre];
...
void	ft_print_xx_conv_3(int fd, t_syntax *syntax, char *str2, int *len)
{
	char	str[(*syntax).width - len[0]];

	if ((*syntax).falgs.minus == 1)
	{
		ft_print_xx_conv_4(fd, syntax, str2, len[1]);
		...
		if ((*syntax).precision == -1)
			ft_print_xx_conv_4(fd, syntax, str2, (*syntax).width - len[0]);
		else
		{
			ft_memset(str, ' ', (size_t)((*syntax).width - len[0]));
			write(fd, str, (size_t)((*syntax).width - len[0]));
...
int		ft_print_xx_conv_2(int fd, t_syntax *syntax, va_list *ap)
{
	unsigned int	arg;
	int				len[2];
	char			*str;
	...
	str = ft_dec_to_hex_2(arg, 19, syntax);
	...
	{
		ft_print_xx_conv_3(fd, syntax, str, len);
		free(str);
		return ((*syntax).width);
	}
	else
		ft_print_xx_conv_4(fd, syntax, str, len[1]);
	free(str);
	return (len[0]);
}
...
int		ft_print_xx_conv(int fd, t_syntax *syntax, va_list *ap)
{
	...
	if ((*syntax).length.hh == 1)
		len = ft_xx_hh(fd, syntax, ap);
	else if ((*syntax).length.h == 1)
		len = ft_xx_h(fd, syntax, ap);
	else if ((*syntax).length.l == 1)
		len = ft_xx_l(fd, syntax, ap);
	else if ((*syntax).length.ll == 1)
		len = ft_xx_ll(fd, syntax, ap);
	else if ((*syntax).length.j == 1)
		len = ft_xx_j(fd, syntax, ap);
	else if ((*syntax).length.z == 1)
		len = ft_xx_z(fd, syntax, ap);
	else
		len = ft_print_xx_conv_2(fd, syntax, ap);
	return (len);
}
